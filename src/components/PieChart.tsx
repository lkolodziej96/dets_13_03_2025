...
const PieChart: React.FC<Props> = ({ data, selectedSector, onSectorSelect }) => {
  ...
  useEffect(() => {
    ...
    const sectorAverages = Object.keys(data[0].sectorScores).reduce(
      (acc, sector) => {
    ...
    const arcs = g.selectAll('.arc').data(pieData);
    ...
    pieData.forEach((d) => {
      ...
      g.append('path')
        ...
        .attr(
          'd',
          `M${posLine[0]},${posLine[1]}L${posLineOuter[0]},${posLineOuter[1]}L${pos[0]},${pos[1]}`,
        )
        ...
        .on('mouseover', (event, labelData) => {
          ...
          g.selectAll('.arc path')
            .filter(function () {
              ...
              const arcData = d3.select(this.parentNode).datum() as d3.PieArcDatum<
                [string, number]
              >;
